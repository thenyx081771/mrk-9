<template>
  <div class="units_wrapper">
    <div class="update_units units_section">
      <el-row class="row_header">
        ...
        <el-col :span="16" class="col_button">
          <uploader fileId="unit" :maxSize="100" :uploadParam="uploadUnitParam" @uploadAfter="uploadUnitsAfter" :url="$api.importUnit" :selfNum="( isCooperate != 0 && self == 0) ? 0 : 1" fileType=".xls,.xlsx" :btnText="{select:$t('units.selectFile'),import:$t('units.importUnitList')}"></uploader>
          <el-button :disabled="(isCooperate != 0 && self == 0)" size="mini" @click="exportUnit">{{$t('units.exportUnitList')}}</el-button>
        </el-col>
      </el-row>
      <el-row>
        <el-col :span="15">
          ...
          <el-tab-pane :label="$t('units.UnitList')" name="1">
            ...
            <el-table :data="unitList" :header-cell-style="{'background':'#f5f7fa'}" style="width: 100%" size="mini">
              ...
              <el-table-column :label="$t('Edit')">
                <template slot-scope="scope">
                  <el-button :disabled="(isCooperate != 0&& self == 0)" size="mini" @click="editUnit(scope.row)">{{$t('units.edit')}}</el-button>
                </template>
              </el-table-column>
            </el-table>
          ...
          <el-tab-pane label="Operation Log" name="3">
            <el-table :data="OperationLog" :header-cell-style="{'background':'#f5f7fa'}" style="width: 100%" size="mini">
              <el-table-column label="Unit Status">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.purchaseStatus">ALL</span>
                    <span v-else>{{scope.row.purchaseStatus}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="Unit Block">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.building">ALL</span>
                    <span v-else>{{scope.row.building}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="Unit Floor">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.floor">ALL</span>
                    <span v-else>{{scope.row.floor}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="Unit Stack">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.stack">ALL</span>
                    <span v-else>{{scope.row.stack}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="Unit Type">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.unitType">ALL</span>
                    <span v-else>{{scope.row.unitType}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column label="Bedroom Type">
                <template slot-scope="scope">
                  <div>
                    <span v-if="!scope.row.bedrooms">ALL</span>
                    <span v-else>{{scope.row.bedrooms}}</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column prop="priceBase" label="Price Base"></el-table-column>
              <el-table-column label="Type">
                <template slot-scope="scope">
                  <div>
                    <span v-if="scope.row.updateType == 1">Discount</span>
                    <span v-else>Quota</span>
                  </div>
                </template>
              </el-table-column>
              <el-table-column prop="type" label="New Price">
                <template slot-scope="scope">
                  <div>
                    <span v-if="scope.row.updateType == 1">{{scope.row.updateValue}}%</span>
      ...
      <div class="edit_price_content">
        <div class="header">
          <el-row>
            ...
            <el-col :span="12">
              <div class="opration">
                <el-button size="mini" :disabled="(isCooperate != 0 && self == 0)" @click="updateUnit">{{$t('update')}}</el-button>
                <el-button size="mini" :disabled="(isCooperate != 0 && self == 0)" @click="dialogOfPrice = false">{{$t('cancel')}}</el-button>
              </div>
            </el-col>
          </el-row>
        </div>
    ...
    <!-- 批量修改价格 -->
    <price-change @priceAdjustment="priceAdjustment" ref="PriceChange"></price-change>
  </div>
</template>
<script>
import uploader from '@/components/uploader'
import PriceChange from './component/priceChange/PriceChange'
export default {
  components: {
    uploader,
    PriceChange,
